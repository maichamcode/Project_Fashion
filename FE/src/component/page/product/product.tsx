...
const product = ({
  ...
  const { data: products, isError, isLoading }: any = useGetProductsNoBlockQuery("");
  console.log(products);

  const productsData = productsort || products;
  const { data: sale, isError: isErrorSeller }: any = useGetProductSellerQuery("");
  const { data: allSales }: any = useGetSaleQuery("");
  ...
  const filteredProducts = productsData?.data?.filter((product: any) => {
    ...
    return false; // Bỏ qua sản phẩm bị chặn
  });
