...
const ButtonTymProductDetail = ({ data }: any) => {
  ...
  const { data: getOneProduct } = useGetOneProductQuery(
  ...
  const initialIsAddedToFavorite =
    localStorage.getItem(`favorite_${data?.data[0]?.product_id}`) === "true";
...
export default ButtonTymProductDetail;
