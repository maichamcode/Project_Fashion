...
import { useFillterProductMaxQuery, useFillterProductMinQuery, useGetAllNewProduct3DaysQuery, useGetSearchProductMutation, useSortProductAtoZQuery, useSortProductZtoAQuery } from "../../../api/product";
import { Form } from "antd";
import DataProductSale from "./dataProductSale";
import DataProductNew3days from "./dataProductNew3Days";

const AllNewProduct3Days = () => {
    const { data: newProducts } = useGetAllNewProduct3DaysQuery("");
    const {
        data: category,
        isError,
        isLoading,
    ...
    // const saleProducts = Product?.data?.filter((product: { sale_id: null; }) => product.sale_id !== null);
                        ...
                        <div className="top_pageniation">
                          <ul>
                            ...
                            <li>3</li>
                                ...
                                {/* product */}
                                {search == null ? (
                                    newProducts ? ( // Render the products from the API if available
                                        <DataProductNew3days
                                            ...
                                            minPrice={minPrice}
                                            maxPrice={maxPrice}
                                            productsort={newProducts} // Render products added within 3 days
                                        />
                                    ) : (
                                        <p>Loading...</p> // Or any loading indicator based on the loading state
                                    )
                                ) : (
                                    <SearchProduct
                                        dataSearch={search}
...
export default AllNewProduct3Days;