...
import { useGetAllOrderQuery, useGetOrderDeleveredQuery, useGetOrderDoneAndCompleteQuery, useGetSearchOrderDoneMutation, useGetSearchOrderShipDoneMutation } from "../../../api/order"
import { Button, Form, Spin, message } from "antd"
import OrderDeliveredDetail from "./OrderDeliveredDetail"
...
const ListOrderDelivered = () => {
    const { data: orderDelivered, isLoading, refetch } = useGetOrderDoneAndCompleteQuery('')
    useEffect(() => {
    ...
    const [getSearch] = useGetSearchOrderShipDoneMutation();
                            ...
                            <tbody className="table-border-bottom-0" style={{ width: "100%" }}>
                                {search ? (
                                    search?.data?.map((order: any) => (
                                        <OrderDeliveredDetail key={order.order_id} data={order} />
                                    ))
                                ) : (
                                    isLoading ? (
                                        ...
                                        (orderDelivered?.orders || []).map((data: any) => <OrderDeliveredDetail data={data} />)
                                    )
                                )}
                            </tbody>
                        </table>
