...
const OrderDeliveredDetail = ({ data }: any) => {
    const discountAmount: any = localStorage.getItem("discountAmount");
    const { data: checkout, refetch } = useGetOneChekoutQuery(data?.checkout_id)
    useEffect(() => {
        refetch();
    }, [refetch]);
    ...
    const [orderShip, { isLoading: shiping }] = useUpdateOrderDoneMutation()
...
export default OrderDeliveredDetail