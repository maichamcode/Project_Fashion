...
import { useGetAllOrderQuery, useGetOrderPenddingQuery, useGetOrderPendingAllQuery, useGetSearchOrderAwaitShipperMutation } from "../../../api/order"
import { Button, Form, Spin, message } from "antd"
import OrderShipDetail from "./OrderShipDetail"
import { useEffect, useState } from "react"


const ListOrderShip = () => {
    const { data: orderpending, isLoading, refetch: pending } = useGetOrderPendingAllQuery('')
    ...
    const [getSearch] = useGetSearchOrderAwaitShipperMutation();
                            ...
                            <tbody className="table-border-bottom-0" style={{ width: "100%" }}>
                                {search ? (
                                    search?.data?.map((order: any) => (
                                        <OrderShipDetail key={order.order_id} data={order} />
                                    ))
                                ) : (
                                    isLoading ? (
                                        ...
                                        (orderpending?.orders || []).map((data: any) => <OrderShipDetail data={data} />)
                                    )
                                )}
