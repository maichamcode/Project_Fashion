...
import { useGetAllOrderQuery, useGetOrderCancellQuery, useGetSearchOrderCancellMutation, useGetSearchOrderMutation } from "../../../api/order"
import { Button, Form, Select, Spin } from "antd"
import OrderCancellDetail from "./OrderCancellDetail"
import { Link, useNavigate } from "react-router-dom"
import { useState } from "react"
import { message } from "antd";


const ListOrderCancell = () => {
    const { data: orderconfirm, isLoading } = useGetOrderCancellQuery(0)
    ...
    const [getSearch] = useGetSearchOrderCancellMutation();
            ...
            <div className="container-xxl flex-grow-1 container-p-y">
                <div className="card">
                    <h5 className="card-header" style={{ display: 'flex' }}>
                        ...
                        <div style={{ width: '15%' }} >
                            <Select style={{ height: '40px', width: '100%' }} defaultValue="cancell"
                                options={[
                                    {
                                        value: 'all',
                                        label: `Tất cả`,
                            ...
                            <tbody className="table-border-bottom-0" style={{ width: "100%" }}>
                                {search ? (
                                    ...
                                    isLoading ? (
                                        ...
                                        (orderconfirm?.orders || []).map((data: any) => <OrderCancellDetail data={data} />)
                                    )
                                )}
                            </tbody>
...
export default ListOrderCancell