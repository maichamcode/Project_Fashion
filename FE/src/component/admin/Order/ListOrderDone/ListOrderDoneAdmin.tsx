...
import { useGetAllOrderQuery, useGetOrderDeleveredQuery, useGetOrderPendingAllQuery, useGetSearchOrderMutation } from "../../../../api/order";
import AdminOrderDetail from "../AdminOrderDetail";
import AdminOrderShipDetail from "../ListOrderShip/ListOrderShipDetail";
import ListOrderDoneAdminDetail from "./ListOrderDoneAdminDetail";

const socket = io('http://localhost:8080');

const ListOrderDoneAdmin = () => {
    const [search, setSearch] = useState<any>(null);
    const [order, setOrder] = useState();
    const [getSearch] = useGetSearchOrderMutation();
    const { data: orders, isLoading, refetch } = useGetOrderDeleveredQuery('');
            ...
            <div className="container-xxl flex-grow-1 container-p-y" >
                <div className="card" >
                    <h5 className="card-header" style={{ display: 'flex' }}>
                        ...
                        <div style={{ width: '15%' }} >
                            <Select style={{ height: '40px', width: '100%' }} defaultValue="received"
                                options={[
                                    {
                                        value: 'all',
                                        label: `Tất cả`,
                                    ...
                                    {search ? ( // Render search results if search is not null
                                        ...
                                        isLoading ? <Skeleton /> : (
                                            orders?.orders.map((order: any) => (
                                                <ListOrderDoneAdminDetail key={order.order_id} data={order} />
                                            ))
                                        )
                                    )}
                                </tbody>
...
export default ListOrderDoneAdmin