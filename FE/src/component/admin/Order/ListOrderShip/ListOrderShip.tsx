...
import { useGetAllOrderQuery, useGetOrderPendingAllQuery, useGetSearchOrderMutation } from "../../../../api/order";
import AdminOrderDetail from "../AdminOrderDetail";
import AdminOrderShipDetail from "./ListOrderShipDetail";
const socket = io('http://localhost:8080');

const ListOrderShipAdmin = () => {
    const [search, setSearch] = useState<any>(null);
    const [order, setOrder] = useState();
    const [getSearch] = useGetSearchOrderMutation();
    const { data: orders, isLoading, refetch } = useGetOrderPendingAllQuery('');
            ...
            <div className="container-xxl flex-grow-1 container-p-y" >
                <div className="card" >
                    <h5 className="card-header" style={{ display: 'flex' }}>
                        ...
                        <div style={{ width: '15%' }} >
                            <Select style={{ height: '40px', width: '100%' }} defaultValue="awaitshipper"
                                options={[
                                    {
                                        value: 'all',
                                        label: `Tất cả`,
...
export default ListOrderShipAdmin