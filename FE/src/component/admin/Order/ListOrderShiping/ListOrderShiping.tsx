...
import { useGetAllOrderQuery, useGetOrderPendingAllQuery, useGetOrderShipingQuery, useGetSearchOrderMutation } from "../../../../api/order";
import AdminOrderDetail from "../AdminOrderDetail";
import AdminOrderShipingDetail from "./ListOrderShipingDetail";
const socket = io('http://localhost:8080');

const ListOrderShiping = () => {
    const [search, setSearch] = useState<any>(null);
    const [order, setOrder] = useState();
    const [getSearch] = useGetSearchOrderMutation();
    const { data: orders, isLoading, refetch } = useGetOrderShipingQuery('');
            ...
            <div className="container-xxl flex-grow-1 container-p-y" >
                <div className="card" >
                    <h5 className="card-header" style={{ display: 'flex' }}>
                        ...
                        <div style={{ width: '15%' }} >
                            <Select style={{ height: '40px', width: '100%' }} defaultValue="shipping"
                                options={[
                                    {
                                        value: 'all',
                                        label: `Tất cả`,
                                ...
                                <tbody className="table-border-bottom-0" style={{ width: "100%" }}>
                                    {search ? ( // Render search results if search is not null
                                        search?.data?.map((order: any) => (
                                            <AdminOrderShipingDetail key={order.order_id} data={order} />
                                        ))
                                    ) : (
...
export default ListOrderShiping